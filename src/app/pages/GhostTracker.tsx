...
import {
  Map as MapIcon, XCircle, CheckCircle, Camera, Navigation, Check, AlertTriangle, Info, Image
} from "lucide-react";
...
export function GhostTracker() {
  ...
  // Nigeria Map outline path for interactive vector fallback
  const nigeriaPath = "2.5,0 19,2 44,1 77.5,5 98,10 100,40 94,60 86,75 61,85 48,97 28,97 11,90 0,75 0,50 7,30 2.5,0";

  // Simulate Google Maps Loader
  useEffect(() => {
    // If we have an API key or window.google is already loaded
    ...
    if (apiKey && mapRef.current) {
      ...
      loader.load().then((google) => {
        ...
        const map = new google.maps.Map(mapRef.current!, {
          center: { lat: 9.0820, lng: 7.4130 }, // Centered in Nigeria
          zoom: 6,
          styles: [
        ...
        mapPins.forEach((pin) => {
          // Convert lon/lat percentages to approximate coordinates in Nigeria
  ...
  const handleCapturePhoto = () => {
    // Simulated Photo Capture Base64
  ...
  const handleSubmitEvidence = async () => {
    ...
    // Save report via global store which interacts with IndexedDB
    await addIssue(
  ...
  return (
    <div className="flex flex-col h-full fade-in">
      ...
      <div 
        ref={mapRef}
        className="relative mx-0 h-[280px] bg-[#080B10] border-b border-white/[0.06] overflow-hidden shrink-0"
        style={{
          backgroundImage:
          ...
          <>
            ...
            {mapPins.map((pin) => (
              <div
                key={pin.id}
                className="absolute"
                style={{ left: `${pin.lon}%`, top: `${pin.lat}%` }}
              >
                <MapPin
                  flagged={pin.flagged}
                  amount={pin.amount}
            ...
            <div className="absolute bottom-3 left-3 bg-[#0E1116]/80 backdrop-blur-sm border border-white/[0.06] rounded-lg px-2.5 py-1.5 pointer-events-none flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-[#8B949E] font-medium uppercase tracking-wider">
                {t("ghostTap")}
              </span>
            </div>
          </>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {selectedPin ? (
          <div className="space-y-4">
            ...
            <CivicCard className="p-4" severity={selectedPin.flagged ? "critical" : "neutral"}>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-white/[0.05] text-left">
                <div>
                  ...
                  <p className="text-[#E8EDF2] text-xs font-bold mt-0.5" style={{ fontFamily: "'DM Mono', monospace" }}>{selectedPin.amount}</p>
                </div>
                <div>
                  <p className="text-[#8B949E] text-[8px] uppercase tracking-widest font-mono">Date Awarded</p>
                  <p className="text-[#E8EDF2] text-xs font-bold mt-0.5" style={{ fontFamily: "'DM Mono', monospace" }}>{selectedPin.date}</p>
                </div>
                <div>
                  <p className="text-[#8B949E] text-[8px] uppercase tracking-widest font-mono">Irregularity Gap</p>
                  <p className={`text-xs font-bold mt-0.5 ${selectedPin.flagged ? "text-[#E3433D]" : "text-[#1E8A5F]"}`} style={{ fontFamily: "'DM Mono', monospace" }}>
                    {selectedPin.gap}%
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4 slide-up">
                ...
                <CivicCard className="overflow-hidden">
                  <div className="px-4 py-2.5 bg-white/[0.01] border-b border-white/[0.06] flex items-center justify-between">
                    <p className="text-[#8B949E] text-[10px] uppercase tracking-widest" style={{ fontFamily: "'DM Mono', monospace" }}>
                      Reality Audit
                    </p>
                  </div>
                ...
                <CivicCard className="p-4">
                  ...
                  {photo ? (
                    <div className="relative rounded-xl overflow-hidden aspect-video bg-[#0E1116] border border-white/[0.06] flex items-center justify-center">
                      {/* Photo Simulator preview */}
                      <div className="absolute inset-0 bg-cover bg-center filter saturate-50 brightness-75" style={{ backgroundImage: `url(${photo})` }} />
                  ...
                  <div className="mt-4 p-3 bg-white/[0.02] border border-white/[0.06] rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#8B949E] text-[10px] font-mono">{t("ghostGPS")}</span>
                    </div>
                    <span className="text-[#1E8A5F] text-[9px] font-bold uppercase tracking-wider flex items-center gap-1">
            ...
            {subView === "uploaded" && (
              <CivicCard className="p-6 text-center space-y-4 slide-up">
          ...
          /* Empty State prompt when map screen first loads */
          <div className="flex flex-col items-center justify-center py-12 text-[#8B949E] text-center space-y-3 px-6">
            ...
            <div>
              <p className="text-xs font-bold text-[#E8EDF2] uppercase tracking-wider">{t("ghostTitle")}</p>
